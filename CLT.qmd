...
```{julia Generating Exponential graphing data}
#| output: false
d = Exponential()
μ = mean(d)
σ = std(d)
...
upper <- sum(graphing$`Exponential Z-Scores` >= 1.96) / 1000000
lower <- sum(graphing$`Exponential Z-Scores` <= -1.96) / 1000000

main <- ggplot(graphing, aes(x = `Exponential Z-Scores`)) +
  geom_histogram(bins = 100, aes(y = after_stat(density))) +
  stat_function(
    fun = dnorm,
    args = list(mean = mean(graphing[[1]]), sd = sd(graphing[[1]])),
    color = "blue",
    linewidth = 1.25
  ) +
  labs(
    title = "Sampling Distribution of Standardized Means for Exponential Population with n = 30",
    x = "Z Score",
    y = "Frequency"
  ) +
  theme_bw() +
...
ggsave(paste0("Poster//Exponential.png"), width = 12, height = 6.75, dpi = 1000)
```
