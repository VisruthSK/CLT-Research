...
bounds <- 0.025 * 20 / 100 * c(-1, 1) + 0.025
...
<!-- sqrt(`Sample Size`)  0.165237    -->

```{r}
n_per_bound <- function(percent) {
  bounds <- 0.025 * percent / 100 * c(-1, 1) + 0.025
...
ggsave("Poster//20_percent.png", width = 15, height = 8.5, dpi = 1000)

plot(x, y, type = "b")
# plot(x, y2, type = "b")

linmanuealmodel <- lm(y ~ x)
summary(linmanuealmodel)

# # multiply this and your percent error bound (0,100), then neg square it to get the constant part of sample size req
# n >= sample_size(20) * skewness^2 for 20% error on tail probs
# n >= sample_size(10) * skewness^2 for 10% error, etc.
# sample_size <- \(percent_bound)
#   (unname(coef(linmanuealmodel)[2]) * percent_bound)^-2

yhats <- sapply(x, \(x) 130.2845 * x^-2)
names(yhats) <- x
sqrt(mean((y - yhats)^2))

sample_size <- \(percent_bound)
  unname(predict(linmanuealmodel, data.frame(x = percent_bound)))

yhats <- sapply(x, \(x) sample_size(x)^(-1 / 2))
names(yhats) <- x
sqrt(mean((y - yhats)^2))

# test <- \(error, skewness) (0.008761 * error)^-2 * skewness^2
```
