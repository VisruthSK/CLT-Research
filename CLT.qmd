...
author: "Visruth Srimath Kandal. Beth Chance, California Polytechnic Department of Statistics"
format: html
...
We chose to look at a variety of distributions with varying levels of skewness, which can be seen in `distributions`, and across various samples sizes as shown in `sample_sizes`. We selected these distributions to showcase the effects of skewness pan distribution as well as within the same distribution.
...
Here we use r = 1,000,000 repetitions to create our sampling distributions of means. Keep in mind that, while Julia is fast, this is still a non-trivial task and will take some time, especially if you aren't explicitly using the new Julia engine to be introduced in Quarto 1.5. It is highly recommended to run the code in a Julia REPL instead of this notebook.
