...
save_as_image(ft, path = "table.png", res = 1000)
...
df |>
  ...
  geom_point(size = 3) +
  geom_label_repel(seed = 0, nudge_x = 1, size = 5) +
  labs(
...
ggsave("Skew_Sample_Size.png", width = 15, height = 8.5, dpi = 500)
...
exponential150 = sampling_distribution(mean, Exponential(), 150, 10_000_000)

graphing = DataFrame(
    "Exponential30" => exponential30,
    "Exponential150" => exponential150,
    "Gamma30" => gamma30,
    "LogNormal50" => lognormal50
)
...
for (col_name in names(graphing)) {
  dist <- str_extract(col_name, "^[A-Za-z]+")
  n <- as.numeric(str_extract(col_name, "\\d+"))
  ...
  ggplot(graphing, aes(x = .data[[col_name]])) +
    ...
    labs(
      title = paste(dist, "Distribution with n =", n),
      x = "Sample Mean",
      y = "Frequency"
    ) +
    theme_bw() +
  ...
  ggsave(paste0(dist, " ", n, ".png"), width = 15, height = 8.5, dpi = 500)
}
```
